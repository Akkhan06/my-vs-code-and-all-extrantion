...
const js_1 = require("../../../line-code-processing/js");
const jsLineCodeProcessing = new js_1.JSLineCodeProcessing();
suite('JS Line Code Processing', () => {
    suite('Classes', () => {
        test('Check class declaration', () => {
            ...
            classLOCs.forEach((classLOC) => {
                assert.strictEqual(jsLineCodeProcessing.doesContainClassDeclaration(classLOC), true);
            });
            nonClassLOCs.forEach((nonClassLOC) => {
                assert.strictEqual(jsLineCodeProcessing.doesContainClassDeclaration(nonClassLOC), false);
            });
        });
        test('Get class name', () => {
            const classLOCs = [
            ...
            classLOCs.forEach((classLOC, index) => {
                assert.strictEqual(jsLineCodeProcessing.getClassName(classLOC), classesNames[index]);
            });
        });
    });
    suite('Functions', () => {
        ...
        test('Check built in function invocation', () => {
            ...
            builtInFunctionInvocationLOCs.forEach((builtInFunctionInvocationLOC) => {
                assert.strictEqual(jsLineCodeProcessing.doesContainsBuiltInFunction(builtInFunctionInvocationLOC), true);
            });
            nonBuiltInFunctionLOCs.forEach((nonBuiltInFunctionLOC) => {
                assert.strictEqual(jsLineCodeProcessing.doesContainsBuiltInFunction(nonBuiltInFunctionLOC), false);
            });
        });
        test('Check named function Declaration', () => {
            namedFunctionsLOCs.forEach((namedFunctionLOC) => {
                assert.strictEqual(jsLineCodeProcessing.doesContainsNamedFunctionDeclaration(namedFunctionLOC), true);
            });
            nonNamedFunctionsLOCs.forEach((nonNamedFunctionLOC) => {
                assert.strictEqual(jsLineCodeProcessing.doesContainsNamedFunctionDeclaration(nonNamedFunctionLOC), false);
            });
        });
        test('Get function name', () => {
            namedFunctionsLOCs.forEach((namedFunctionLOC, index) => {
                assert.strictEqual(jsLineCodeProcessing.getFunctionName(namedFunctionLOC), namedFunctionsNames[index]);
            });
        });
        test('Assignment of function to a variable', () => {
            functionsAssignmentsLOCs.forEach((functionsAssignmentsLOC) => {
                assert.strictEqual(jsLineCodeProcessing.isFunctionAssignedToVariable(functionsAssignmentsLOC), true);
            });
        });
        ...
        `,
            ...
            objectFunctionCallLOCs.forEach((objectFunctionCallLOC) => {
                assert.strictEqual(jsLineCodeProcessing.isObjectFunctionCall(objectFunctionCallLOC), true);
            });
            functionsAssignmentsLOCs.forEach((functionsAssignmentsLOC) => {
                assert.strictEqual(jsLineCodeProcessing.isObjectFunctionCall(functionsAssignmentsLOC), false);
            });
        });
    ...
    test('Assignment of object literal to a variable', () => {
        ...
        objLiteralAssignmentLOCs.forEach((objLiteralAssignmentLOC) => {
            assert.strictEqual(jsLineCodeProcessing.isObjectLiteralAssignedToVariable(objLiteralAssignmentLOC), true);
        });
        nonObjLiteralAssignmentLOCs.forEach((nonObjLiteralAssignmentLOC) => {
            assert.strictEqual(jsLineCodeProcessing.isObjectLiteralAssignedToVariable(nonObjLiteralAssignmentLOC), false);
        });
    });
    test('Assignment of an array to a variable', () => {
        ...
        arrayAssignmentLOCs.forEach((arrayAssignmentLOC) => {
            assert.strictEqual(jsLineCodeProcessing.isArrayAssignedToVariable(arrayAssignmentLOC), true);
        });
        nonArrayAssignmentLOCs.forEach((nonArrayAssignmentLOC) => {
            assert.strictEqual(jsLineCodeProcessing.isArrayAssignedToVariable(nonArrayAssignmentLOC), false);
        });
    });
});
//# sourceMappingURL=lineCodeProcessing.test.js.map